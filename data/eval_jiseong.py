...
def check_performance_entity(answers, texts):
	...
	for i in range(len(answers)):
		...
		o_file = open('result.txt', 'a+')
		try:
			o_file.write(text + '\n\n')
		except:
			o_file.write(text.encode('utf-8') + '\n\n')

		try:
			o_file.write(json.dumps(answer, ensure_ascii=False) + '\n\n')
		except:
			o_file.write(json.dumps(answer, ensure_ascii=False).encode('utf-8') + '\n\n')

		try:
			o_file.write(json.dumps(elu_result, ensure_ascii=False) + '\n\n')
		except:
			o_file.write(json.dumps(elu_result, ensure_ascii=False).encode('utf-8') + '\n\n')
		o_file.close()
